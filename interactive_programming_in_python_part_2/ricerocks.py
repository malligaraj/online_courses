...
def group_collide(other_obj, group):
    global explosion_group
    for obj in set(group):
        if obj.collide(other_obj):
            explosion_obj = Sprite(obj.pos, obj.vel, 0, obj.angle_vel, explosion_image, explosion_info, explosion_sound)
            explosion_group.add(explosion_obj)
            group.discard(obj)
            return True
    return False

def group_group_collide(obj_group_1, obj_group_2):
    collisions = 0
    for obj in set(obj_group_1):
        if group_collide(obj, obj_group_2):
            collisions += 1
            obj_group_1.discard(obj)    
...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            explosion_index = [self.age % self.pos[0], (self.age // self.pos[0]) % self.pos[1]]
...
def rock_spawner():
    ...
    if started:
        if len(rock_group) <= 12:
            if dist(my_ship.get_position(), rock_pos) > 3 * my_ship.get_radius():

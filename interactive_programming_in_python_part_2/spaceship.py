...
class Ship:
    ...
    def draw(self,canvas):        
        if self.thrust:
            image_center = [self.image_center[0] + self.image_size[0], 
                            self.image_center[1]]
        else:
    ...
    def shoot(self):
        ...
        MISSILE_SPEED = 5
        forward = angle_to_vector(self.angle)
        missile_position = [self.pos[0] + self.radius * forward[0], 
                            self.pos[1] + self.radius * forward[1]]
        missile_velocity = [self.vel[0] + forward[0] * MISSILE_SPEED, 
...
def draw(canvas):
    ...
    canvas.draw_text('Score: ' + str(score), (600, 50), 30, 'Gray', 'monospace')
    canvas.draw_text('Lives: ' + str(lives), (50, 50), 30, 'Gray', 'monospace')
...
def rock_spawner():
    ...
    angle_vel = random.choice([1, -1]) * 0.05
